...
export interface ILeadRepository {
  findAll(filters?: LeadFilterQuery): Promise<LeadRecord[]>;
  findById(id: number): Promise<LeadRecord | null>;
  findByEmail(email: string): Promise<LeadRecord | null>;
  create(data: NewLeadRecord): Promise<LeadRecord>;
  update(id: number, data: Partial<NewLeadRecord>): Promise<LeadRecord | null>;
  delete(id: number): Promise<boolean>;
}

export class LeadRepository implements ILeadRepository {
  async findAll(filters?: LeadFilterQuery): Promise<LeadRecord[]> {
    try {
      const conditions = [];
      ...
      const sortColumn = filters?.sortBy ? leads[filters.sortBy] : leads.createdAt;
  ...
  async findByEmail(email: string): Promise<LeadRecord | null> {
    try {
      ...
      const results = await db.select().from(leads).where(ilike(leads.email, normalizedEmail)).limit(1);
      return results[0] || null;
    } catch (error) {
  ...
  async create(data: NewLeadRecord): Promise<LeadRecord> {
    try {
      ...
      if (pgError?.code === '23505') {
        throw new DatabaseError('A lead with this email address already exists', { cause: error });
      }
